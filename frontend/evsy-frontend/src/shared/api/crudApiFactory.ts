...
export function createCrudApi<T, R = T>(endpoint: string) {
  return {
    ...
    create: (data: Partial<R>) => api.post<T>(`/${endpoint}`, data).then((r: AxiosResponse<T>) => r.data),
    update: (id: number | string, data: Partial<R>) => api.put<T>(`/${endpoint}/${id}`, data).then((r: AxiosResponse<T>) => r.data),
