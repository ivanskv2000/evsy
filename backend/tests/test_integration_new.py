...
def test_complex_event_creation_integration(auth_client):
    ...
    field_data = FieldCreate(
        name="Source", description="Event source", field_type=FieldType.string
    )
...
def test_event_deletion_cascade_integration(auth_client, db):
    ...
    f_res = auth_client.post(
        "/v1/fields/", json={"name": "f", "description": "d", "field_type": "string"}
    )
    field_id = f_res.json()["id"]

    # 2. Create an event linked to them
    e_res = auth_client.post(
        "/v1/events/",
        json={
            "name": "To Delete",
            "description": "d",
